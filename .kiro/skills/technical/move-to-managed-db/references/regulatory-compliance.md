...
> **Confidence labels**: claims marked **[Regulatory]** are well-established regulatory facts whose verbatim primary text (KISA/PIPC standards) should be re-confirmed against the current "개인정보의 안전성 확보조치 기준" before publishing a customer plan. Framework-level claims are cited to authoritative legal/AWS sources.

---

## 1. 개인정보보호법 (PIPA — Personal Information Protection Act)
...
- The **Enforcement Decree** requires resident registration numbers (주민등록번호) to be **retained encrypted at rest**, regardless of controller size.

**Fields that must be encrypted** **[Regulatory]**:

| Field | Requirement |
|-------|-------------|
| 주민등록번호 (RRN) and other 고유식별정보 (passport #, driver's license #, foreigner reg. #) | Encrypted at rest **and** in transit |
| 비밀번호 (passwords) | **One-way (irreversible)** encryption — hashing, not decryptable |
| 바이오정보 (biometric data) | Encrypted |
| 신용카드번호 / 계좌번호 (card / bank account numbers) | Encrypted |
...
- Baseline: retain access/processing records (접속기록) for **at least 1 year**.
...
## 2. 전자금융감독규정 (Electronic Financial Supervision Regulation — FSC)

For financial institutions, the FSC regime historically mandated **strict network separation (망분리)** between systems handling sensitive data and external networks, plus access control, encryption, audit logging, and separation of duties.
...
- Compensating controls required: pre-screening by the **Financial Security Institute (금융보안원)**, strict authentication and **least-privilege access**, **network-layer encryption**, monitoring of critical information flows, and **semi-annual (6-month) compliance audits**.
...
**[Regulatory]** Exact ISMS-P control IDs should be confirmed against the current 인증기준 catalog before a certification engagement.
...
| 망분리 / financial sensitivity | Private subnets, no IGW for DB tier, PrivateLink endpoints, Direct Connect/VPN for on-prem; document separation in the To-Be architecture Compliance Matrix. |
...
- The **granular KISA specifics** (exact SEED/ARIA/AES/SHA-256 algorithm table and the precise 1-vs-2-year retention thresholds) are well-established but should be **verified against the current "개인정보의 안전성 확보조치 기준"** before being stated as fact to a customer.
