...
On a self-managed database (on-prem or EC2), Korean security tools attach in one of four ways:
...
| **Inline gateway / proxy** ("게이트웨이") | Clients connect to the proxy; the proxy connects to the DB. All policy is enforced in the proxy | 🟢 **Survives** — deploy the proxy in the VPC in front of the RDS endpoint |
...
> ⚠️ **Vendor naming — get this right with the customer.** **Chakra Max is a WareValley (웨어밸리) product.** **Petra (페트라) and Petra Cipher are SINSIWAY (신시웨이) products.** **DBSafer is PNPSECURE (피앤피시큐어).** **D'Amo is Penta Security (펜타시큐리티).** These are frequently conflated; getting the vendor wrong in a customer meeting costs credibility.

---

## 1. Access-Control & Audit Tools (네트워크/게이트웨이 계열)
...
### 1.1 Chakra Max (샤크라맥스) — WareValley (웨어밸리)
...
### 1.2 DBSafer (디비세이퍼) — PNPSECURE (피앤피시큐어)
...
| **On-prem architecture** | **Primary mode is Gateway/Proxy** — "네트워크를 통해 유입/유출되는 모든 데이터를 통제하는 Gateway(Proxy) 구성." A **Server Agent** additionally catches bypass/direct (console) connections that don't traverse the gateway. Client component "PC Assist." |
...
### 2.1 Petra Cipher (페트라 사이퍼) — SINSIWAY (신시웨이)
...
### 2.2 D'Amo / D.AMO (디아모) — Penta Security (펜타시큐리티)
...
### 2.3 CUBE-One (큐브원) — Comtrue Technology (컴트루테크놀로지)
...
> ⚠️ **Inferred completion.** The source instruction for this table was truncated after
> `| Modes | Ag`. The rows below mirror the IBM Guardium structure and this document's DAS
> integration thesis (§4.1, where AWS documents Imperva DAS integration for Aurora PostgreSQL).
> **Verify against Imperva's current AWS deployment guide before using in a customer plan.**
...
> ⚠️ **Inferred completion.** The source instruction was truncated after the first item
> (`- [ ] Latency impact measured (ta`). The remaining items below follow the gateway/proxy
> migration concerns raised elsewhere in this document (§1.2 DBSafer, §4.2 access control).
> **Review before using in a customer plan.**
...
- **RDS Proxy** — managed proxy that can **enforce IAM authentication**, integrates **Secrets Manager**, and provides private VPC-only access. This is the native analog of the Korean "게이트웨이" mode. ⚠️ It does **not** give the fine-grained SQL-command-level policy/masking the Korean tools provide — for those, run the vendor's **gateway mode** in the VPC alongside it.
...
- [ ] **Is this a financial workload subject to 망분리?** If it touches unique-ID / personal-credit info, plan the stricter separated regime (private subnets, no IGW, PrivateLink, Direct Connect).
