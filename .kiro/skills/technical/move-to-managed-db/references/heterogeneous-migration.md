...
> **Critical Reference Document** — The original skill covered only homogeneous MySQL/MariaDB/PostgreSQL moves. This document extends coverage to **every engine RDS/Aurora supports**, to **heterogeneous migrations** (Oracle/SQL Server → Aurora), and to the **Korean-market source engines** (Tibero, CUBRID, Altibase) that have **no native AWS migration tooling**.
...
| **Aurora MySQL-Compatible** | ~5× MySQL throughput, storage to 256 TiB, up to 15 replicas, Global Database. |
...
- [ ] **Application SQL remediation** — embedded SQL in the app that uses source-specific dialect.
